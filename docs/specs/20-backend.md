...
> **Статус:** черновик спеки v0.1 · зависит от `docs/specs/00-canon.md` (канон v1.1), реализует техническую сторону всех системных спек `01`–`18`.
...
- **~17 доменов, ~75 таблиц.** Идентичность/соц-граф, ферма (грядки/станки/склад/рецепты), **животные**, прогрессия (стафф/know-how/постройки/Farm Value), календарь, спрос, кооп/потлак/помощь/town-projects, ярмарка/конкурсы, серверный ивент/Appetite Meter/лиги, коллекции/**ачивки**/**декор+неон**, монетизация (Route Pass/Prize Machine/покупки), **фуражинг+почта**, **смена/валидация**, **переезды-голосования**, удержание (Daily Specials/стрик/vacation-sitter/win-back/**онбординг-стейт**), инфра (config/леджеры/**idempotency**/античит/аудит).
- **Валюты — ровно 4 (канон §2.1, DECISIONS-B K11).** `bucks\|dimes\|tickets\|ribbons` — только они в `currency_ledgers`. Технические счётчики (Know-How Points, staff tokens, scrap, Route Miles) — **игровое состояние в отдельных state-таблицах**, не валюты леджера.
...
| `vacation_until` | `timestamptz` | `mech_gone_fishin` активен если > now() (offset-таймеры, B7) |
| `last_migrated_at` | `timestamptz` | кулдаун переезда 2 недели (`mech_moving_truck`); **скип за `◉` невозможен** (12-O1) |
| `town_joined_at` | `timestamptz` | момент входа в текущий город; мин. 3 дня до нового переезда |
...
| `quality` | `smallint` **not null default 0** | для блюд/культур (стек на (key,quality)); **0 = «без качества»/N/A** (sentinel, не nullable) |
| `expires_at` | `timestamptz` | для скоропорта (лимит Icehouse) |

Unique: `(farm_id, item_key, quality)`. **`quality` NOT NULL с sentinel `0`** — чтобы стеки не двоились через `NULL≠NULL` (иначе против дюпа нужен `UNIQUE NULLS NOT DISTINCT`, PG15+; выбран sentinel как переносимый). Индекс: `(farm_id, item_class)`. Лимиты хранения (Silo/Icehouse) валидируются в RPC, не колонкой.

> Прочие unique-ключи схемы — на NOT NULL колонках (`(player_id, recipe_key)`, `(player_id, postcard_key)`, `(stall_id, slot_index)`, `(provider, provider_txn_id)` и т.д.), nullable-коллизий не создают. Единственный nullable-в-unique был `inventory.quality` — закрыт sentinel'ом.
...
**`player_know_how` (Ноу-хау игрока, `player_know_how`)** — состояние ветвей знаний. Know-How Points — **не валюта** (канон §2.1, DECISIONS-B K11: валют ровно 4), а игровое состояние; в `currency_ledgers` не хранится.

| Колонка | Тип | Заметки |
|---|---|---|
| `player_id` | `uuid` PK FK→`players.id` `on delete cascade` | 1:1 с игроком |
| `points` | `bigint` not null default 0 | накопленные Know-How Points (тратятся на `research_start`) |
| `active_slots` | `int` not null default 1 | параллельных исследований (2-й слот — за `dimes`, `13-progression.md`) |
| `spent_points` | `bigint` not null default 0 | сумма вложенного (для Farm Value / аудита) |

Начисление/списание `points` — только в RPC (`research_start` тратит, наградные джобы начисляют), append-лог движений — в `audit_logs`, **не в `currency_ledgers`**.

**`player_state_counters` (Служебные счётчики, state)** — не-каноничные технические счётчики, вынесенные из леджера по K11 (не валюты): `player_id uuid` PK FK→players `cascade` · `staff_tokens bigint` default 0 (апгрейд стаффа) · `scrap bigint` default 0 (`⚙`, скрап дублей Prize Machine). `miles` (Route Miles) живёт в `route_pass_progress.miles` (§3.2.10) — тоже state, не валюта. Движение этих счётчиков — только через RPC, лог — в `audit_logs`.
...
| `season_id` | `uuid` FK→`route_pass_seasons.id` | текущий сезон (8 нед, R7 — мастер `01-core-loop.md`) |
...
| `entry_open` | `timestamptz` | Пн 00:00 UTC |
| `entry_close` | `timestamptz` | Пт 23:59 UTC |
| `voting_open` | `timestamptz` | Сб 00:00 UTC (совпадает с открытием ярмарки — голоса собираются на площади) |
| `voting_close` | `timestamptz` | Вс 12:00 UTC (конец окна голосования = закрытие прилавков, K8) |
| `announce_at` | `timestamptz` | оглашение (после судейства, ≈Вс 12:00+) |
| `state` | `text` default `'entry'` | `entry\|voting\|judged` |

Unique: `(town_id, week_index, contest_key)`. **Тайминг (K8, окно ярмарки 36 ч):** приём заявок Пн→Пт; **голосование Сб 00:00→Вс 12:00** (внутри окна ярмарки); судейство — **на конце окна голосования** (`contest_judge` в Вс 12:00+, а не на Сб 00:00), чтобы голоса игроков успевали учесться до оглашения. Переходы фаз двигают cron `contest_open_voting` (Сб 00:00) и `contest_judge` (Вс 12:00).
...
**`route_pass_seasons` (Сезон Route Pass)** — **8 недель** (R7 — мастер `01-core-loop.md`/`15-monetization.md`): `id uuid` PK · `season_index int` unique · `theme_key text` · `start_week int` · `end_week int`.
...
| `id` | `bigint` PK identity | |
| `player_id` | `uuid` FK→`players.id` `on delete cascade` | |
| `currency` | `text` not null check (currency in ('bucks','dimes','tickets','ribbons')) | **ровно 4 каноничные** (§2.1, K11); служебные счётчики (kh_points/staff_tokens/scrap/miles) сюда НЕ пишутся — они state (`player_know_how`, `player_state_counters`, `route_pass_progress`) |
...
Индексы: `(player_id, currency, at)`, unique `(idempotency_key)` where not null. Баланс кошелька — вью `wallet_balances` = last `balance_after` per (player, currency); либо материализованная таблица `wallets(player_id, currency, balance)` обновляемая триггером на леджер. **Ни одна RPC не меняет баланс мимо леджера** — это ядро античита экономики. Леджер держит **только 4 каноничные валюты** (check-constraint выше).

> Примечание (DECISIONS-B K11): `scrap` (`⚙`), Know-How Points, staff tokens, Route Miles — **не валюты** и в `currency_ledgers` не хранятся. Это игровое состояние в state-таблицах: `player_state_counters.scrap`/`.staff_tokens`, `player_know_how.points`, `route_pass_progress.miles`. Канон фиксирует ровно 4 валюты (§2.1) — расширять enum нельзя.
...
**`idempotency` (Универсальный гард идемпотентности)** — для наградных/оркестрационных действий, чей ключ **не** ложится на `(town, week, anchor)` (`processed_anchors` покрывает только фазовые якоря). Джобы `contest_judge`, `contest_open_voting`, `season_rollover`, `merge_check`, `dau_recalc`, `push`, `streak_freeze`, `foraging_respawn`, `winback`, `fair_tick` и любой Edge с внешним эффектом сначала пытаются `INSERT ... ON CONFLICT DO NOTHING` сюда и работают только при успешной вставке.

| Колонка | Тип | Заметки |
|---|---|---|
| `scope` | `text` not null | неймспейс действия (`contest_judge\|season_rollover\|push\|fair_tick\|…`) |
| `key` | `text` not null | составной ключ в scope (напр. `contest_id`, `season_id`, `player:template:day`, `lot:tick_window`) |
| `at` | `timestamptz` not null default now() | момент первичного захвата |
| `result` | `text` | `ok\|error` (для ретрая упавших) |

PK составной: `(scope, key)`. GC старых записей (>90 дн) — `chat_gc`.

#### 3.2.12 Домен: Животные (`03-animals.md`)

**`animals` (Животное, `Animal`)** — скот/птица в Barn/Coop (канон §3.8, `bld_barn`/`bld_coop`).

| Колонка | Тип | Заметки |
|---|---|---|
| `id` | `uuid` PK | |
| `farm_id` | `uuid` FK→`farms.id` `on delete cascade` | |
| `species` | `text` not null | `cow\|chicken\|pig\|dairy_goat\|turkey\|…` (`03-animals.md`) |
| `pet_name` | `text` | кличка (RPC `rename_pet`) |
| `housing_key` | `text` not null | `bld_barn\|bld_coop\|bld_apiary` (пасека — у Farmhand, 03-O1/O3) |
| `housing_level` | `int` not null default 1 | 1–10 (K4, мастер `13-progression.md`) — влияет на quality_tier |
| `affection` | `int` not null default 0 | очки привязанности; ★-порог даёт Blue Ribbon-качество (`03-animals.md §3.5`) |
| `fed_at` | `timestamptz` | последнее кормление (Clara `staff_clara` −цикл) |
| `product_ready_at` | `timestamptz` | когда готов продукт (серверный таймер, как `machine_jobs`) |
| `product_key` | `text` | что даёт (`milk\|egg\|…`) |
| `quality` | `smallint` not null default 1 | от affection+housing+staff+affection_gift (см. §3.5 03-animals) |
| `state` | `text` not null default `'idle'` | `idle\|hungry\|producing\|ready\|sleepy_pen` |

Индекс: `(farm_id, state)`, `(product_ready_at)`. Unique `(farm_id, pet_name)` where pet_name not null.

**`holding_pen` (Загон-буфер, Sleepy Pen)** — при переполнении вместимости постройки лишние животные ждут (E3, «не теряются»): `id uuid` PK · `farm_id uuid` FK→farms `cascade` · `animal_id uuid` FK→animals `cascade` unique · `since timestamptz`.

RPC: `feed_animal(animal_ids)` (списывает корм, ставит `product_ready_at`), `collect_animal_product(animal_ids)` (`now()≥product_ready_at`, quality от affection/housing, кладёт на склад с лимитом), `rename_pet(animal_id, name)`, `affection_gift(animal_id, gift_key)` (**серверный инкремент affection, идемпотентность недельная** — 1 подарок на животное/неделя, ключ `(player, animal, week)` в `idempotency`). Взаимодействие с Vacation: в отпуске `product_ready_at` смещается offset-механизмом (§3.2.15/B7), животные не «голодают» в минус.

#### 3.2.13 Домен: Фуражинг и почта (`08-mail-foraging.md`)

**`foraging_points` (Точки фуражинга)** — общая карта города, пул обновляется ежедневно (`mech_foraging`).

| Колонка | Тип | Заметки |
|---|---|---|
| `id` | `uuid` PK | |
| `town_id` | `uuid` FK→`towns.id` `on delete cascade` | |
| `point_type` | `text` not null | `mushroom\|berry\|fishing\|wild_beehive` (4 типа; 4 фиксированы, `tp_water_tower` даёт 5-й улей — 08 §3.2.6) |
| `pool_remaining` | `int` not null | текущий остаток (атомарный `UPDATE … WHERE pool_remaining>0`) |
| `pool_max` | `int` not null | пул ≥ личный лимит × ожидаемое число сборщиков (08 ОВ-5) |
| `respawn_at` | `timestamptz` | следующий респавн (cron `foraging_respawn` 06:00) |

Unique: `(town_id, point_type)`. Realtime `town:{}:foraging` транслирует появление/исчерпание.

**`forage_daily` (Дневной лимит сбора)** — анти-фарм: **суммарный лимит на игрока по типу точки/день** (не по инстансу — иначе 6×8 обход, 08 §3.2.3): `player_id uuid` · `point_type text` · `game_day date` · `count int`. PK `(player_id, point_type, game_day)`.

RPC: `forage_claim(point_id)` / `forage_collect(point_id)` — атомарный декремент пула (гонка → мягкое «уже собрали последнее», B3/F6), проверка `forage_daily` против кэпа; `fish_cast()` — **серверный RNG редкости с гарантированным Common** (нет пустого броска, P3); дроп объявляет сервер (античит). Cron `foraging_respawn` `0 6 * * *` пополняет пулы.

**`mail_catalog_weeks` (Недельный каталог, `mech_mail_catalog`)** — ротация ассортимента: `id uuid` PK · `town_id uuid` FK→towns `cascade` · `week_index int` · `items jsonb` (`[{item_key, price, stock, rarity}]`, детерм. seed, anti-repeat). Unique `(town_id, week_index)`. Генерит cron `mail_generate` Пн 00:00.

**`mail_orders` (Заказ почтой, `Postman Pete`)** — доставка 8–20 ч (R19):
...
| `deliver_at` | `timestamptz` not null | +8–20 ч (`08-mail-foraging.md`, скип за `dimes` кэп ◉5 — R3-исключение) |
| `delivered` | `bool` default false | |
| `collected` | `bool` default false | |

Индекс: `(player_id, collected)`, `(deliver_at)`. Лимиты: Rare ≤3, Decor ≤1, Tools ≤5 в неделю; **≤5 заказов в пути** одновременно — enforced counting-запросом в RPC. RPC: `mail_order(item_key)` / `mail_speedup(order_id)` (скип за `◉`, кэп ◉5) / `mail_claim(order_ids)`.

#### 3.2.14 Домен: Смена, стрик, спецблюда, ачивки, декор

**Валидация смены (`09-fair.md`)** — активная смена у прилавка (`ui_shift`) не имеет своей таблицы состояния (эфемерна на клиенте), но её **итог валидируется сервером**: RPC `shift_submit(shift_log)` реконструирует Tips/FairScore/tickets/FP из **фактически списанного стока**, игнорируя клиентские числа (см. §3.4.1). Лимит **3 смены/окно ярмарки + кулдаун 2 ч** между сменами — counting по `fair_sales`/`audit_logs`; атомарный вклад FP в `event_weeks.meter_fp`.

**`player_streaks` (Стрик завсегдатая, `mech_regular_streak`)** — дневной стрик (`16-retention.md`, E2).

| Колонка | Тип | Заметки |
|---|---|---|
| `player_id` | `uuid` PK FK→`players.id` `on delete cascade` | |
| `streak_days` | `int` not null default 0 | текущая длина |
| `best_streak` | `int` not null default 0 | рекорд (для win-back восстановления 50%) |
| `state` | `text` not null default `'active'` | `active\|frozen\|broken` |
| `last_credited_day` | `date` | последний зачтённый игровой день (UTC) |
| `freezes_this_month` | `int` not null default 0 | 2 бесплатные заморозки/мес (`16-retention.md §3.3`) |
| `insured_until` | `timestamptz` | страховка за `🎟` (RPC `streak_insure`) |

RPC `streak_insure()` (тратит `tickets`), `streak_check()` (≥2/3 Daily Specials за день → +1), cron `streak_freeze` переводит невыполненные в `frozen` (не обнуляет — E2).

**`daily_specials` (Спецблюда дня, `mech_daily_special`)** — ротация 3 фокус-задач (`16-retention.md`): `id uuid` PK · `player_id uuid` FK→players `cascade` · `game_day date` · `template_key text` · `focus text` · `target int` (скейл от `farm_value`) · `progress int` default 0 · `done bool` default false · `yesterday_focus text` (anti-repeat, пул 32 шаблона). Unique `(player_id, game_day, template_key)`. Прогресс инкрементится **внутри профильных RPC** (harvest/craft/sell/…), генерация — джобом ролловера дня.

**`player_achievements` (Ачивки, `17-collections.md`)**: `player_id uuid` FK→players `cascade` · `ach_key text` · `unlocked_at timestamptz`. PK `(player_id, ach_key)` (идемпотентность). Серверная детекция из релевантных RPC.

**`player_decor` (Декор дайнера/двора, `17-collections.md`)**: `id uuid` PK · `player_id uuid` FK→players `cascade` · `decor_key text` · `slot text` (интерьер/двор/фасад) · `placed bool` · `layout jsonb`. Unique `(player_id, decor_key)`. Вклад декора в Farm Value — **через `farm-value-recalc` с капом Σ(косметика+коллекции)≤0.15×FV** (13-ОВ7/8, R2).

**`player_neon_sign` (Неон-вывеска, `ui_neon_builder`)**: `player_id uuid` PK FK→players `cascade` · `config jsonb` (сегменты/цвета/анимация) · `updated_at timestamptz`. RPC `neon_save(config)` (валидация набора сегментов против владения).

RPC: `decor_purchase`/`decor_place`/`decor_save`, `neon_save`, `fair_tent_upgrade` (апгрейд палатки — `fair_stalls.stall_level`/`display_slots`, шаг цены — `09-fair.md §3.6`), `recipe_experiment` (RNG-открытие секретного рецепта, Mystery Plate, D8 канон; дневной кэп в `boosters_daily`).

#### 3.2.15 Домен: Переезды-голосования, отпуск, онбординг, win-back

**`migration_proposals` (Предложение переезда, `12-migration.md`)** — Town Merge / Street Caravan (`mech_town_merge`/`mech_street_caravan`).

| Колонка | Тип | Заметки |
|---|---|---|
| `id` | `uuid` PK | |
| `kind` | `text` not null | `town_merge\|street_caravan` |
| `scope_id` | `uuid` not null | `town_id` (merge) или `street_id` (caravan) |
| `target_town_id` | `uuid` FK→`towns.id` | куда переезд/слияние |
| `opened_at` | `timestamptz` not null | |
| `closes_at` | `timestamptz` not null | Caravan 72 ч / Merge — календарная неделя Пн 00:00–Вс 23:59 UTC (12-migration §3.3.3) |
| `state` | `text` default `'voting'` | `voting\|passed\|failed\|executed` |

**Частичный unique** `(scope_id) where state='voting'` — одно активное голосование на город/стрит. Кворумы: Caravan 60%, Merge 50%+ явка ≥40% (K5-смежно). Исполнение — идемпотентно на rollover (`idempotency(scope='migration_exec', key=proposal_id)`).

**`migration_votes` (Голос за переезд)**: `id uuid` PK · `proposal_id uuid` FK→migration_proposals `cascade` · `player_id uuid` FK→players · `vote text` (`yes\|no`) · `at timestamptz`. **Unique `(proposal_id, player_id)`** (античит накрутки). RPC `migration_propose`/`migration_vote`. **Ускорение соц-кулдаунов за `dimes` запрещено** (D11, DECISIONS-B 12-O1) — нет RPC/поля для скипа окна голосования или кулдауна переезда.

**Moving cooldown (`12-migration.md`)** — на `players` добавлены колонки: `last_migrated_at timestamptz` (кулдаун 2 недели `mech_moving_truck`), `town_joined_at timestamptz` (мин. 3 дня до нового переезда). Проверки — в `migrate-farm`; **скип за `◉` невозможен** (12-O1).

**`neighbor_sits` (Присмотр за фермой, Neighbor Sitter)** — во время отпуска (`16-retention.md`): `id uuid` PK · `host_id uuid` FK→players `cascade` · `sitter_id uuid` FK→players · `game_day date`. **Unique `(host_id, game_day)`** — первый смотритель дня получает валюту-награду, остальные — косметическое «спасибо» (анти-манекен). RPC `neighbor_sit(host_id)` (гард `sitter≠owner`, арбитр гонки), суммарный кэп награды на ферму-в-отпуске/сутки.

**Vacation (`16-retention.md`, `mech_gone_fishin`)** — `players.vacation_until` (уже есть). RPC `vacation_start()` / `vacation_end()` работают по **offset-механизму таймеров** (не «логическая заморозка», см. B7): при старте для активных таймеров запоминается `remaining = ready_at − now()`; при выходе `ready_at = now() + remaining`. Мин. кулдаун между активациями 24–48 ч (анти-манекен).

**`onboarding_state` (Стейт онбординга, `18-onboarding.md`)**: `player_id uuid` PK FK→players `cascade` · `t_day int` (день туториала) · `ftue_complete bool` default false · `skipped bool` default false · `flags jsonb` (пройденные шаги). RPC `onboarding_advance`/`onboarding_complete`/`onboarding_skip` (серверный анти-фрод флаг). **Гейт: игрок не попадает в `towns.dau_7d` до `ftue_complete`** (чистота метрик активности).

**`winback_state` (Win-back волны, `16-retention.md §3.7`)**: `player_id uuid` PK FK→players `cascade` · `last_wave text` · `wave_sent_at timestamptz` · `best_streak_snapshot int`. Cron `winback_scan` по `last_seen_at`, идемпотентность `(player, wave)`; при возврате — восстановление 50% `best_streak` и `mech_grand_opening (win-back)` 48 ч (R1).
...
|---|---|---|
| Приватные игрока (`inventory`, `plots`, `machine_jobs`, `recipes*`, `staff_*`, `know_how_nodes`, `player_know_how`, `player_state_counters`, `route_pass_progress`, `prize_*`, `boosters_daily`, `regulars_club`, `farm_value_snapshots`, `animals`, `holding_pen`, `player_streaks`, `daily_specials`, `player_achievements`, `player_decor`, `player_neon_sign`, `onboarding_state`, `mail_orders`, `forage_daily`, `winback_state`) | `player_id = auth.uid()` (или через `farm→player`) | **`false`** — только RPC/Edge |
| Публичные-в-городе — **предикат SELECT задаётся пер-таблично** (не лумпить, у части нет колонки `town_id`): `towns` → `id = (select town_id from players where id=auth.uid())`; `streets`/`street_members` → `street_id in (select id from streets where town_id = my_town)` (через `street→town`, у `street_members` прямого `town_id` нет); `server_calendars`/`market_weeks`/`town_projects`/`contests`/`event_weeks`/`mail_catalog_weeks`/`foraging_points` → `town_id = my_town`; `versus_matches` → `town_a = my_town OR town_b = my_town` (нет `town_id`) | **`false`** |
| Соседские (read) (`farms.layout` соседа, `farm_visits`, `ribbons_wall`, `toys`, `collections`, `postcards` — витринные) | членство в том же городе/стрите (`host/player town = my_town`) | **`false`** |
| Кооп/вклад (`orders`, `order_contributions`, `potlucks`, `event_contributions`, `contest_entries`, `contest_votes`, `migration_proposals`, `migration_votes`, `neighbor_sits`, `mentorships`) | город/стрит игрока (пер-таблично через `town_id`/`street_id`) | **`false`** (вклад/голос — через RPC) |
...
| `research_start(node_key)` | старт узла know-how | `{node}` | предки изучены; хватает Know-How Points (`player_know_how.points`, **не валюта**); активных исследований ≤ `player_know_how.active_slots` (2-й слот — за `dimes`) |
| `staff_assign(staff_key, post)` | назначить пост | `{assignment}` | стафф нанят; пост валиден; заменяет прежнего |
| `staff_upgrade(staff_key)` | апгрейд жетонами | `{level}` | хватает staff tokens (`player_state_counters.staff_tokens`, **не валюта**) |
...
| `wallet_get()` | баланс | `{balances}` | только свой; 4 каноничные валюты |
| `streak_check()` | тик стрика | `{streak_days, state}` | считает ≥2/3 Daily Specials за игровой день UTC; пишет `player_streaks` |
| `streak_insure()` | страховка стрика | `{insured_until}` | тратит `tickets`; ставит `insured_until` (E2) |
| `feed_animal(animal_ids)` | кормление | `{fed:int}` | корм есть; ставит `product_ready_at` (Clara −цикл) |
| `collect_animal_product(animal_ids)` | сбор продукта | `{items}` | `now()≥product_ready_at`; quality от affection/housing; лимит склада |
| `rename_pet(animal_id, name)` | кличка | `{ok}` | владелец; уникальна на ферме |
| `affection_gift(animal_id, gift_key)` | подарок питомцу | `{affection}` | **1/животное/неделя** (idem `(player,animal,week)`); серверный инкремент |
| `forage_claim(point_id)` / `forage_collect(point_id)` | сбор с точки | `{item}` | атомарный декремент пула (F6); кэп `forage_daily` по типу/день |
| `fish_cast()` | рыбалка | `{catch}` | серверный RNG редкости, **гарантированный Common** (P3) |
| `mail_order(item_key)` | заказ каталогом | `{order}` | ≤5 в пути; лимиты Rare3/Decor1/Tools5; `deliver_at`=+8–20ч |
| `mail_speedup(order_id)` | ускорить доставку | `{deliver_at}` | скип за `◉`, кэп ◉5 (R3-исключение) |
| `mail_claim(order_ids)` | забрать посылку | `{items}` | `now()≥deliver_at`; лимит склада |
| `shift_submit(shift_log)` | итог смены у прилавка | `{tips, fair_score, tickets, fp}` | **сервер реконструирует из фактически списанного стока**, клиентские числа игнорит; ≤3 смены/окно + кулдаун 2ч; кэп `🎟` 5/неделя (R10) |
| `migration_propose(kind, target_town)` | предложить переезд | `{proposal}` | 1 активное на город/стрит; **без скипа за `◉`** (12-O1) |
| `migration_vote(proposal_id, vote)` | голос | `{tally}` | 1 голос/предложение; окно голосования открыто |
| `vacation_start()` / `vacation_end()` | отпуск-режим | `{vacation_until}` | offset-таймеры (B7); кулдаун 24–48ч |
| `neighbor_sit(host_id)` | присмотр | `{ok}` | `sitter≠owner`; 1 награда/ферма/день; арбитр гонки |
| `onboarding_advance/complete/skip()` | шаг туториала | `{state}` | серверный анти-фрод; не в `dau_7d` до `ftue_complete` |
| `decor_purchase/place/save`, `neon_save(config)` | декор/неон | `{ok}` | владение; вклад в FV через `farm-value-recalc` (кап 15%) |
| `fair_tent_upgrade()` | апгрейд палатки | `{stall_level, display_slots}` | хватает `bucks`; шаг цены — `09-fair.md §3.6` |
| `recipe_experiment(inputs)` | эксперимент (секретка) | `{result}` | RNG-открытие (Mystery Plate, D8); дневной кэп в `boosters_daily` |
...
| `contest-judge` | cron `contest_judge` в **конце окна голосования** (Вс 12:00 UTC, K8) | `{contest_id}` | `{ranked}` | считает `npc_score`, агрегирует голоса игроков (собранные Сб 00:00→Вс 12:00), `final_score`, ранги, выдаёт Blue Ribbon; идемпотентно по `idempotency(scope='contest_judge')` |
...
| `migrate-farm` | голосование/выбор игрока | `{player_id, target_town}` | `{ok}` | проверка кулдаунов (`last_migrated_at` 14 дн, `town_joined_at` 3 дн — **скип за `◉` запрещён, 12-O1**); атомарный перенос строк фермы между city-партициями; тикет-компенсация town-projects (D12, курс 50:1, R15) |
...
|---|---|---|---|
| `phase_tick` | `*/5 * * * *` (каждые 5 мин) | сверяет `now()` с окнами, двигает `server_calendars.phase`, шлёт broadcast | `(town_id, week, phase)` (per-город, чтобы фаза одного города не гасила прогон другого) |
...
| `fair_tick` | `*/15 * * * *` в окне | Edge `fair-tick` пассив продаж | `idempotency(scope='fair_tick', key=(lot_id,tick_window))` |
| `contest_open_voting` | `0 0 * * 6` (Сб 00:00) | перевод конкурсов `entry→voting` (после `entry_close` Пт 23:59) | `idempotency(scope='contest_voting', key=contest_id)` |
| `contest_judge` | `0 12 * * 0` (Вс 12:00, конец голосования) | Edge `contest-judge`: `voting→judged`, учёт голосов игроков, Blue Ribbon | `idempotency(scope='contest_judge', key=contest_id)` |
| `fair_close` | `0 12 * * 0` (Вс 12:00) | закрыть ярмарку, финализ лотов, **возврат непроданного стока** (см. B16) | `(town,week,'fair_close')` |
...
| `season_rollover` | на границе сезона | soft-reset лиг (25% перенос), новый Route Pass сезон | `idempotency(scope='season_rollover', key=season_id)` |
| `merge_check` | `0 6 * * *` (ежедневно) | Edge `merge-check` DAU-сканы | `idempotency(scope='merge_check', key=game_day)` |
| `dau_recalc` | `0 1 * * *` | пересчёт `towns.dau_7d`, `last_seen` агрегаты | `idempotency(scope='dau_recalc', key=game_day)` |
| `chat_gc` | `0 3 * * *` | чистка `chat_messages` >30 дн, `fair_sales`/`audit_logs` >90 дн (гипотеза) | idem (без ключа — чистка идемпотентна по природе) |
| `push_daily` | `0 * * * *` (ежечасно) | Edge `push-dispatch` для due-триггеров с тихими часами | `idempotency(scope='push', key=(player,template,game_day))` |
| `streak_freeze` | `0 0 * * *` (00:00 игрового дня) | перевести невыполненные стрики в `frozen` (E2) | `idempotency(scope='streak_freeze', key=(player,game_day))` |
| `foraging_respawn` | `0 6 * * *` (06:00 ежедневно) | респавн пулов точек фуражинга (`08-mail-foraging.md`) | `idempotency(scope='forage_respawn', key=(town,game_day))` |
| `mail_generate` | `0 0 * * 1` (Пн 00:00) | недельная ротация каталога (детерм. seed, anti-repeat) | `(town,week,'mail_gen')` |
| `winback_scan` | `0 5 * * *` (05:00 ежедневно) | скан `last_seen_at`, постановка win-back волн (`16-retention.md`) | `idempotency(scope='winback', key=(player,wave))` |
...
| **Подделка Farm Value для лиг** | `farm_value` пересчитывает только `farm-value-recalc` (Edge/service_role); клиент не пишет. Лиги читают `farm_value`+вклад, не спенд (канон G1). Σ(косметика+коллекции) капится 15% (13-ОВ7/8). |
| **Накрутка итога активной смены** | `shift_submit` не принимает клиентские Tips/FairScore: сервер реконструирует их из **фактически списанного стока** (`fair_sales`/резерв лотов); ≤3 смены/окно + кулдаун 2ч; кэп `🎟` 5/неделя (R10). |
| **Абуз соц-кулдаунов за `◉`** (переезды/голосования/менторство) | **Нет пути ускорить за `dimes`** (D11, DECISIONS-B 12-O1): кулдауны `last_migrated_at`/`town_joined_at` и окна голосований проверяются в `migrate-farm`/`migration_vote`; ни RPC, ни поля скипа не существуют — анти-абуз миграций перед ивентами. |
| **Накрутка голосов за переезд** | `migration_votes (proposal_id, player_id)` unique; смурф-фильтр `device_fingerprints`; кворумы по явке. |
| **Манекен-ферма ради награды смотрителя** | `neighbor_sits (host_id, game_day)` unique — 1 оплачиваемая награда/ферма/день; мин. кулдаун 24–48ч между `vacation_start`. |
| **Обход анти-фарма фуражинга через инстансы** | Лимит `forage_daily` — **суммарный по типу точки/день**, не по инстансу (08 §3.2.3); пул декрементится атомарно. |
| **Пустой бросок рыбалки / рейс без лута** | Сервер гарантирует минимум Common (`fish_cast`) и ≥1 строку лута экспедиции (07 §4.2) — «нет провала» (P3). |
...
| Moving Van (Фургон переезда) кулдаун | 2 недели | `mech_moving_truck` (канон §3.13, K9: отображаемое имя — Moving Van; ключ не переименован) |
...
| Тик фаз (`phase_tick`) | `(town_id, week, phase)` | `processed_anchors` (пер-город) |
| Покупка IAP | `(provider, provider_txn_id)` | `purchases` (unique) |
| Судейство конкурса | `(scope='contest_judge', key=contest_id)` | `idempotency` (PK) |
| Ротация сезона/лиг | `(scope='season_rollover', key=season_id)` | `idempotency` (PK) |
| Пуш-отправка | `(scope='push', key=player:template:day)` | `idempotency` (PK) |
| Заморозка стрика | `(scope='streak_freeze', key=player:day)` | `idempotency` (PK) |
| Win-back волна | `(scope='winback', key=player:wave)` | `idempotency` (PK) |
| Подарок питомцу | `(scope='affection_gift', key=player:animal:week)` | `idempotency` (PK) |
| Исполнение переезда | `(scope='migration_exec', key=proposal_id)` | `idempotency` (PK) |
| Тик пассива лота | `(scope='fair_tick', key=lot_id:tick_window)` | `idempotency` (PK) |
...
| `ui_shift` (Смена у прилавка) | конфиг смены, `fair_lots` резерв | `shift_submit` (сервер реконструирует итог) | `town:fair` |
...
| `ui_moving_truck` (Moving Van / Фургон переезда) | `migration_proposals`,`migration_votes` | `migration_propose`/`migration_vote`, Edge `migrate-farm` | `street:board` |
...
| Удержание | `docs/specs/16-retention.md` | `daily_specials`,`player_streaks`,`streak_check`/`streak_insure`/`streak_freeze`, `neighbor_sits`, `winback_state`, `push-dispatch`, тихие часы, offset-таймеры отпуска |
| Коллекции | `docs/specs/17-collections.md` | `collections`,`toys`,`postcards`,`ribbons_wall`,`player_achievements`,`player_decor`,`player_neon_sign`; вклад декора в FV с капом 15% (13-ОВ7/8) |
| Онбординг | `docs/specs/18-onboarding.md` | `onboarding_state`, гейт `dau_7d` до FTUE-complete |
| Мультиплеер-синк (шардинг/партиции/Realtime-протокол) | **этот док** §3.5 (Realtime), §3.6 (cron), §3.9 (миграции) + `docs/specs/21-client.md` (клиентский сетевой слой) | нет отдельного файла: детали синка распределены между 20-backend и 21-client (K1: `17-` = `17-collections.md`) |
...
| B7 | **Игрок в отпуске (`vacation_until`)** | **Offset-механизм** (не «логическая заморозка»): `vacation_start` для активных таймеров запоминает `remaining = ready_at − now()`; `vacation_end` восстанавливает `ready_at = now() + remaining`. Пока `vacation_until > now()` — ферма `status='vacation'`, продукция/рост не «портятся» и не капают в минус; сосед-смотритель (`neighbor_sits`) имеет ограниченные права помощи (E6). Таймеры не тикают «вникуда» и не сбрасываются. |
...
| B15 | **Голосование за собственную заявку конкурса** | Отклоняется в `contest_vote` (гипотеза: нельзя за себя); unique-гард против повтора. Голоса собираются в окне `voting` (Сб 00:00→Вс 12:00), судейство — на его конце (K8). |
| B16 | **Закрытие ярмарки с непроданным стоком** | `fair_close` (Вс 12:00) возвращает непроданное `qty_listed − qty_sold` из зарезервированного обратно в `inventory` (с проверкой лимита Silo/Icehouse); излишек сверх лимита предлагается в подарок/потлак, не теряется (E3, P3). |
| B17 | **Отрицательный баланс `◉` после потреблённых бустеров** | Списание `◉` в `currency_ledgers` не уходит ниже нуля (check на проекции баланса в RPC); при попытке — трата блокируется (`rejected` + `audit_logs`), баф не выдаётся. Флаг аккаунта на ручной ревью при повторах (согласовано с `15-monetization.md` M-E8). |

---

## 8. Открытые вопросы

> Закрыто в Фазе B (DECISIONS-B): **под-валюты в леджере** — `[ЗАКРЫТО — K11]` know_how_points/scrap/staff_tokens/miles **не валюты**, вынесены из `currency_ledgers` в state-таблицы (`player_know_how`, `player_state_counters`, `route_pass_progress`); enum валют — ровно 4 (§3.2.11). **Финал ивента** — `[ЗАКРЫТО — K7]` Вс 20:00 UTC круглогодично (канон §2.3/§8 п.1); cron `event_settle` `0 20 * * 0` фиксирован. **Формула pity** — `[ЗАКРЫТО — K2]` Rare≤10 / Chase≤40, дроп 68/24/6.5/1.5% (мастер `15-monetization.md`; канон §8 п.6); схема хранит `pulls_since_rare/chase`, пороги — в `game_configs`.

1. **Шардинг городов по региону/языку.** `towns.region_tag` введён гипотезой (нужен для Town Browser, `12-migration.md` O2), но канон не фиксирует региональное шардирование. Все города в одном пуле или партиционируются по региону? Влияет на `migrate-farm` (кросс-регион переезд) и матчмейкинг versus.
2. **Партиционирование по городу.** Предполагается партиция крупных таблиц (`inventory`,`fair_sales`,`event_contributions`,`chat_messages`) по `town_id` ради `migrate-farm` и масштаба. На каком пороге DAU включать декларативные партиции Postgres — нужен нагрузочный тест (гипотеза: >5k активных ферм).
3. **Тик пассива ярмарки 15 мин vs event-driven.** `fair-tick` крон каждые 15 мин (`09-fair.md` OQ) — компромисс. Альтернатива: ленивое вычисление продаж при чтении прилавка (без крона). Согласовать стоимость Supabase-крона vs латентность лога продаж.
4. **Пуш-провайдер и тихие часы.** `push-dispatch` требует внешний провайдер (Web Push/FCM/APNs); тихие часы — по таймзоне клиента (`tz_offset_min`) или серверные? (`16-retention.md` OQ). Влияет на планировщик `push_daily`.
5. **Материализация кошелька.** `wallets` как триггер-обновляемая таблица vs вью поверх `currency_ledgers` (last `balance_after`). Таблица быстрее на чтение, но требует строгого триггер-гарда против рассинхрона. Решить по нагрузке HUD-запросов.
6. **Число постов стаффа (Yard).** Канон §8 п.5 держит открытым 4-й пост `Yard`. Схема `staff_assignments.post` допускает 4 значения; если канон свернёт `Yard`, миграция сузит enum.
7. **RNG-детерминизм для аудита.** Prize Machine и генерация спроса используют серверный seed. Нужно решить: хранить ли seed каждой круты в `prize_pulls` для полной воспроизводимости аудита (privacy/объём) — или достаточно результата. Гипотеза: хранить seed только при спорных кейсах.
8. **Дедлайн Co-op Orders vs окно экспедиций.** Канон §8 п.4 держит открытым возможный конфликт дедлайна Чт 23:59 с окном экспедиций (Ср). Схема нейтральна (`orders.deadline` = якорь), но при изменении канона поменяется генерация в `market-generate`.
9. **Storage-квоты Photo Mode.** `photo-upload` в Supabase Storage — нужен лимит на игрока (гипотеза: 50 скриншотов, авто-GC старых) и модерация UGC. Не покрыто каноном.
10. **Курс скипа доставки Mail Catalog.** `mail_speedup` за `◉` — исключение из мастер-формулы скипа (R3, ◉1=4ч, кэп ◉5); финальные числа централизуются в `14-economy.md §3.8`. Схема нейтральна (`deliver_at`), но курс/потолок нужно свести с эконом-спекой.
