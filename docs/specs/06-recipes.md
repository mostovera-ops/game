...
> **Статус:** черновик v0.1 для ревью. **Канон обязателен:** `docs/specs/00-canon.md` (версия 1.0) — при конфликте следовать канону, расхождение — в §8.
> **Источник концепта:** `docs/concept/sunnyside-deck.html`, слайд 06 «Кухня».
> **Скоуп файла:** каталог блюд, станки, mastery, сеты Blue Plate Special, секретные рецепты-эксперименты. Экономика (финальные цены/тайминги) централизуется в `docs/specs/12-economy.md` — все числа здесь помечены `(гипотеза)`.
...
Пример из дека сохранён как канон-иллюстрация: Cherry Pie à la Mode, ★★★☆☆ (34 готовки — между порогами ★2 и ★3, отображается прогресс-баром до ★3), база $75 → фактическая цена с текущим прогрессом ≈$85, при ★3 (30 готовок) — $92 (соответствует деку: «база $75, c mastery $92» — дек описывает уже достигнутый ★3).
...
Сет = Main (главное блюдо) + Side (гарнир) + Drink (напиток), собранные в **Fair Stall (`ui_fair_stall`)** или на Counter Shift (`ui_shift`) как единый лот. Бонус сета: `+18% (гипотеза)` к сумме цен компонентов при подаче как сет, растёт до `+25%` при mastery ★3+ у всех трёх компонентов одновременно («синергия трёх звёзд»). Сеты не обязаны быть одного тира — тир сета = тир самого дорогого компонента, но бонус считается от фактической суммы цен (с учётом mastery каждого).
...
Как в деке: `Toast → Sandwich → Club`, `Burger → Cheeseburger → Deluxe Burger`, `Apple Pie → Cherry Pie → à la Mode`, `Shake → Malt → Banana Split`. Технически дерево — это цепочка рецептов, где рецепт N+1 требует **готовое блюдо N как один из ингредиентов** (не сырьё напрямую), так что апгрейд физически «кладёт тост в сэндвич». Полный список деревьев — см. пометку `[树N]` в таблице §4.2.
...
| S3 | Whipped Cream | Взбитые сливки | Butter Churn | Milk×2 | 8 мин | T1 |
| S4 | Pickles | Соленья | Prep Counter | Cucumber×3 (доп. ингредиент, гипотеза) | 15 мин | T1 |
| S5 | Coleslaw | Коулслоу | Prep Counter | Lettuce×2, Milk×1 | 10 мин | T1 |
...
| S10 | Vanilla Custard («Пломбир») | Ванильный пломбир | Ice Cream Maker | Milk×3, Vanilla Bean×1 | 15 мин | T2 |
...
| S17 | Roux | Ру (основа гамбо) | Stockpot | Wheat×1, Butter×1 | 15 мин | T3 |
...
| 10 | Honey-Pecan Toast | Тост с мёдом и пеканом | T3 | Grill | Bread×1, Honey×1, Pecan×2 | 15 мин | 48 | шт:Tennessee |
...
| 21 | Cherry Pie à la Mode | Вишнёвый пай а-ля мод | T3 | Bake Oven | Cherry Pie×1, Vanilla Custard×1 | 50 мин | 92 (при ★3) / база 75 | Ур.8 `[древо2, финал]`, канон-иллюстрация дека |
...
| 48 | Cream Soda | Крем-содовая | T1 | Soda Fountain | Milk×1 | 4 мин | 5 | Ур.2 |
| 49 | Fresh Lemonade Float | Лимонадный флоат | T1 | Soda Fountain | Home Lemonade×1, Whipped Cream×1 | 6 мин | 8 | Ур.2 `[древо4-подветка]` |
...
| 53 | Strawberry Malt | Клубничный малт | T3 | Soda Fountain | Classic Milkshake×1, Strawberry×2 | 25 мин | 55 | Ур.7 `[древо4]` |
| 54 | Southern Coffee | Южный кофе | T3 | Coffee Machine | Coffee Bean×2 | 20 мин | 45 | Ур.7 |
| 55 | Honey Cream Coffee | Кофе со сливками и мёдом | T3 | Coffee Machine | Coffee Bean×2, Honey×1, Whipped Cream×1 | 25 мин | 58 | шт:Tennessee |
...
| 59 | Banana Split (напиток-десерт гибрид, см. также №75) | Банана-сплит | T4 | Soda Fountain | Strawberry Malt×1, Vanilla Custard×1, Caramel Sauce×1 | 110 мин | 215 | Ур.13 `[древо4, финал]` |
...
| # | EN | RU | Тир | Станок | Ингредиенты | Время | Цена $ | Открытие |
|---|---|---|---|---|---|---|---|---|
| 61 | Vanilla Scoop | Шарик пломбира | T1 | Ice Cream Maker | Vanilla Custard×1 | 5 мин | 6 | Ур.2 |
...
| 71 | Praline Bread Pudding | Пекановый брэд-пудинг | T4 | Bake Oven | Bread×2, Pecan×3, Caramel Sauce×1 | 110 мин | 225 | шт:Tennessee, Ур.12 |
...
| 91 | Hushpuppies Basket | Корзинка хашпаппи | T3 | Fryer | Hushpuppy Batter×3 | 25 мин | 50 | Ур.7 |
...
| 100 | Georgia Pecan Pie Plate Special | Тарелка «Джорджия»: курица + пекан + персик | T5 | Fryer | Chicken×2, Pecan×2, Georgia Peach×2 | 200 мин | 520 | шт:Georgia, Ур.17 |
...
| 28 | Georgia Grand Plate | Большая тарелка Джорджии | T5 | Georgia Pecan Pie Plate Special (#100) | Praline Bread Pudding (#71) | California Citrus Cooler (#60) | +25 |
...
|---|---|---|
| Экономика/баланс | `docs/specs/12-economy.md` | Финальные цены/тайминги (эта спека даёт гипотезы по канон-таблице §2.2); синки Bucks. |
| Ферма/производство (грядки, животные, сырьё) | спека фермы/производства (номер уточняется другим агентом) | Реальные тайминги роста сырья T1–T5, доп. сырьё сверх канонных хайлайтов (см. §4.1 сноску), лимиты грядок/стойл. |
| Постройки | спека построек (`bld_kitchen`, `bld_icehouse`, `bld_silo`) | Уровни кухни → слоты станков и очередь; лимиты хранения готовых блюд. |
| Стафф | спека стаффа (`staff_bruno`, `staff_rosalind`, `staff_marty`) | Модификаторы: −10% время (Bruno), +1★ прирост mastery выпечки (Rosalind), +1 к партии гриля (Marty). |
| Know-How | спека know-how, ветка `kh_cookery` | Разблокировка станков/слотов очереди сверх базовых, ускорение mastery-прироста. |
| Demand Board / рынок спроса | спека рынка спроса (`ui_demand_board`) | ±15–30% к категориям недели; перенасыщение категории — мягкое удешевление (canon E7); категории спроса §4.3 — прямой вход туда. |
| Экспедиции / штаты | спека экспедиций (`st_illinois`…`st_california`) | Разблокировка ингредиентов и рецептов по штатам (столбец «Открытие» в §4.2). |
| Ярмарка / смена / прилавок | спека ярмарки (`ui_fair_stall`, `ui_shift`) | Использование Blue Plate Special сетов, продажа блюд по спросу. |
| NPC / нарратив | спека NPC (`npc_nana_opal`) | Стартовый рецепт, нарративные карточки-награды, менторские подарки. |
| Co-op Orders / Potluck | спека кооп-заказов, потлак-спека | Блюда/сеты как валюта заказа («20 пирогов, 40 бургеров»); использует те же Recipe ID. |
| Ивенты | спека ивентов (`ev_glutton`, `ev_big_festival`) | Блюда как единицы «сытости» для Appetite Meter; секретки Sec.21 как лимитированный ивент-тег. |
| Косметика/неон | `ui_neon_builder` | Разблокировка декора за mastery ★5 карточки (см. §3.3). |
...
| R3 | Неудачный эксперимент — жалко потраченных редких T5-ингредиентов | Эксперимент **запрещает** использовать сырьё тира выше текущего максимального открытого станка минус 1 (защита от случайной траты трюфеля на T1-станке); плюс лимит «1 попытка в сутки» снижает частоту потерь. |
...
2. **Доп. сырьё сверх канонных хайлайтов** (Лимон, Лук, Огурец, Курица, Пекан, Сом, Каджун-специи, Раки, Бришкет, Калифорнийский цитрус) — необходимо для покрытия 112 блюд, но канон §2.2 даёт только 5 хайлайтов на тир, не исчерпывающий список. Нужно решение: фиксировать этот список в канон или сократить каталог блюд до строго канонных 25 ингредиентов (сильно урезает южную/морскую кухню).
3. **Banana Split (№59) как гибрид категории Напитки/Десерты** — двойной тег для Demand Board технически нетипичен; нужно решение спеки рынка спроса — считать его отдельной подкатегорией или закрепить только за Напитками (как в дереве блюд дека).
...
5. **Партия крафта = ×1 baseline** — нужно подтверждение у спеки построек/стаффа, что апгрейд партии происходит только через `staff_marty`/know-how, а не через уровень станка напрямую (иначе двойной источник роста тиража).
6. **Цена и время блюд в этой спеке — гипотезы, выведенные вручную** по формуле «балланс вокруг тирового бейзлайна §2.2 ±40%»; финальная калибровка централизуется в `12-economy.md` и может изменить конкретные цифры без изменения структуры каталога.
7. **Смешанные ингредиенты-заменители до открытия штата** (например, №33 Classic Burger без говядины, №52/88/62 с временными заменителями мёда/кофе/яблок) — решение введено этой спекой ради ранней доступности рецепта по уровню без ожидания экспедиции; нужно решение нарратив-спеки, не выглядит ли это нелепо («бургер без говядины») — альтернатива: сдвинуть такие рецепты на уровень открытия нужного сырья.
8. **Rumor Board (`ui_rumor_board`)** как отдельный UI-экран не описан ни в деке, ни в каноне — нейминг-кандидат этой спеки; требует решения UI/социальной спеки, интегрировать ли в существующий городской чат/ленту вместо отдельного экрана.
9. **Apple/Cucumber/Cocoa/Macaroni** как ингредиенты в отдельных рецептах (№18, S4, №51, Sec.12) введены ad hoc для конкретных блюд и не сведены в общий список доп. сырья §4.1 — нужно либо добавить в реестр допов, либо заменить эти рецепты на канонные ингредиенты.
