...
export const hero = {
  ...
  moving: false,
  /**
   * Точка, к которой герой доворачивается, стоя на месте. Её ставит
   * <Interactions>, когда герой дошёл: сперва повернуться лицом, потом делать.
   */
  faceAt: null as { x: number; z: number } | null,
  /** Довернулся ли на faceAt. Без запроса — true: ждать нечего. */
  facing: true,
}

/** Насколько точно нужно смотреть на объект, чтобы начать действие. */
export const FACE_EPS = 0.08 // рад, ≈4.5°
